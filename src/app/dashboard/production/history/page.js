...
import {
  FlaskConical, Search, Calendar,
  Download, Loader2, Trash2, ChevronDown, TrendingUp
} from 'lucide-react'
...
export default function ProductionHistoryPage() {
  ...
  async function fetchHistory() {
    ...
    let query = supabase
      ...
      .select(`
        id, entry_date, batch_no, quantity, notes,
        entered_at, ip_address,
  ...
  async function deleteEntry(id) {
    if (!confirm('Delete this production entry? Raw material stock will be restored.')) return
    const { error } = await supabase.from('daily_production').delete().eq('id', id)
    if (error) toast.error('Failed to delete')
    else { toast.success('Deleted — stock restored'); fetchHistory() }
  }
  ...
  records.forEach(r => {
    ...
    if (!totalByProduct[name]) totalByProduct[name] = { total: 0, unit }
  ...
  function exportCSV() {
    ...
    const header = ['Date', 'Product', 'Category', 'Batch No', 'Quantity', 'Unit', 'Notes', 'Entered By', 'Time']
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-subtitle">{totalBatches} batches in selected range</div>
        </div>
      ...
      {/* Filters */}
      <div className="filters-bar card" style={{ padding: 16, marginBottom: 20 }}>
      ...
      {!loading && Object.keys(totalByProduct).length > 0 && (
        <div className="summary-strip">
          <div className="summary-strip-label">
            <TrendingUp size={13} /> Total Production
          </div>
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{ fontWeight: 500 }}>{r.products?.name}</span>
                    </div>
                  </td>
                  <td>
                    {r.products?.category
                      ? <span className="badge badge-blue">{r.products.category}</span>
                      : <span className="text-faint">—</span>
                    }
                  </td>
                  <td>
                    <span className="badge badge-orange">Batch {r.batch_no}</span>
                  </td>
                  <td>
                    <span style={{
                  ...
                  <td style={{ color: 'var(--text-2)', fontSize: 13 }}>
                    {r.profiles?.full_name || <span className="text-faint">—</span>}
                  </td>
      ...
      <style jsx>{`
        ...
        .summary-chip-name { color: var(--text-2); }
        .summary-chip-val {
