                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              const msg = encodeURIComponent(
                                `Dear ${dist.name},\n\nWe have received your payment of ₹${parseFloat(p.amount).toLocaleString('en-IN', {minimumFractionDigits:2})} on ${new Date(p.entry_date).toLocaleDateString('en-IN', {day:'numeric',month:'short',year:'numeric'})} via ${p.payment_mode?.toUpperCase()}${p.reference_no ? ` (Ref: ${p.reference_no})` : ''}.\n\nThank you for your payment!\n— MilkyFeast 🙏`
                              )
                              const num = dist.phone.replace(/\D/g,'')
                              window.open(`https://wa.me/${num.startsWith('91') ? num : '91'+num}?text=${msg}`, '_blank')
                            }}
                            style={{ display:'flex', alignItems:'center', gap:4, padding:'5px 10px', borderRadius:'var(--r-sm)', background:'rgba(37,211,102,0.08)', border:'1px solid rgba(37,211,102,0.3)', color:'#128c7e', fontSize:12, cursor:'pointer', fontFamily:'var(--font-body)' }}>
                          ...
                          {/* Button 2: Share payment receipt PDF */}
                          <button
                            title="Share Payment Receipt PDF"
                            onClick={async () => {
                              const dist = distributors.find(d => d.id === p.distributor_id)
                              try {
                                const { generateSalaryReceiptPDF: _unused, generateSaleBillPDF: _unused2, ...pdfUtils } = await import('@/lib/utils/pdf')
                                const { generatePaymentReceiptPDF, downloadPDF, generateInvoiceNo } = pdfUtils
                                if (!generatePaymentReceiptPDF) {
                                  // Fallback: just download a simple text version
                                  toast('PDF receipt: use the WhatsApp message button for now', { icon: 'ℹ️' })
                                  return
                                }
                                const doc = await generatePaymentReceiptPDF({
                                  receiptNo:    generateInvoiceNo('MF-PR'),
                                  date:         new Date(p.entry_date).toLocaleDateString('en-IN', {day:'numeric',month:'long',year:'numeric'}),
                                  distributor:  dist || { name: 'Distributor' },
                                  amount:       parseFloat(p.amount),
                                  paymentMode:  p.payment_mode,
                                  referenceNo:  p.reference_no,
                                  notes:        p.notes,
                                })
                                downloadPDF(doc, `MilkyFeast_Payment_Receipt_${dist?.name || ''}_${p.entry_date}.pdf`)
                                toast.success('Receipt PDF downloaded — attach in WhatsApp')
                              } catch(err) {
                                toast.error('PDF error: ' + err.message)
                              }
                            }}
                            style={{ display:'flex', alignItems:'center', gap:4, padding:'5px 10px', borderRadius:'var(--r-sm)', background:'var(--blue-dim)', border:'1px solid rgba(37,99,235,0.3)', color:'var(--blue)', fontSize:12, cursor:'pointer', fontFamily:'var(--font-body)' }}>
